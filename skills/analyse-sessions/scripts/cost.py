...
Examples:
  ...
  python cost.py --cwd C:\\path\\to\\project   # one project, all time
...
def main() -> int:
    ...
    if args.json:
        ...
        if group_limit is not None:
            report["groups"] = limit_groups(report["groups"], args.by, group_limit)
    ...
    if by == "day":
        ...
        for rec in S.iter_records(s.path):
            ...
            if not isinstance(cost_data, dict):
                cost_data = {}
            try:
                per_model[model] += float(cost_data.get("total") or 0)
...
def print_grand_total(summaries) -> None:
    ...
    if summaries:
        ...
        last = max((s.last_at or s.started_at for s in summaries
                    if s.last_at or s.started_at), default=None)
        if first and last:
...
def limit_groups(groups: dict, by: str, limit: int) -> dict:
    """Apply --limit consistently to JSON and table group output."""
    items = list(groups.items())
    if by == "day":
        items.sort(key=lambda kv: kv[0])
    else:

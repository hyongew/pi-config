...
type AskQuestionStatus = "answered" | "cancelled" | "unavailable";
type AskQuestionMode = "text" | "single-select" | "multi-select";

interface AskQuestionResultDetails {
...
// UI popups are process-global. Share a lock through globalThis so this tool
// serializes with other popup-style extensions loaded in the same process.
...
function buildStructuredResult(
	...
	return {
		...
		answers,
		message,
	} as AskQuestionResultDetails;
}

function cancelledResult(question: string, mode: AskQuestionMode, context?: string) {
...
function unavailableResult(question: string, mode: AskQuestionMode, message: string, context?: string) {
	return {
		content: [{ type: "text" as const, text: message }],
...
function buildResult(question: string, context: string | undefined, mode: AskQuestionMode, answers: AskAnswer[]) {
	let text: string;
	if (mode === "text") {
...
function createAbortableDone<T>(signal: AbortSignal | undefined, done: (result: T) => void, abortResult: T): (result: T) => void {
	let finished = false;
	...
	const onAbort = () => finish(abortResult);
	if (signal?.aborted) {
		finish(abortResult);
	} else {
...
async function askSingleChoice(
	...
	return ctx.ui.custom<AskAnswer | null>((tui: any, theme: any, _kb: any, done: (result: AskAnswer | null) => void) => {
		...
		const finish = createAbortableDone<AskAnswer | null>(signal, done, null);
		...
		function render(width: number): string[] {
			// requestRender() does not necessarily invalidate on terminal resize.
...
export default function askQuestion(pi: ExtensionAPI) {
	pi.registerTool({
		...
		promptGuidelines: [
			"Use ask-question to ask exactly one question per tool call.",
			"If you need answers to multiple questions, make multiple separate ask-question tool calls instead of combining them into one prompt.",
			'In TUI mode, users can always select "Other" to provide custom text input when options are provided.',
			"Set ask-question multiSelect: true only when you need multiple answers to the same question.",
			'If you recommend a specific option, make it the first option in the list and add "(Recommended)" at the end of the label.',
			"Prefer ask-question over guessing when requirements, preferences, or implementation choices are unclear.",
			"Use ask-question when multiple valid implementation paths exist and the preferred path depends on user choice.",
		],
		parameters: AskQuestionParams,
		// UI tools can only handle one active interactive call at a time.
		executionMode: "sequential",

		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			const options = normalizeOptions(params.options);
			const context = params.details?.trim() || undefined;
			const mode: AskQuestionMode = options.length === 0 ? "text" : params.multiSelect ? "multi-select" : "single-select";

			if (signal?.aborted) {
				return cancelledResult(params.question, mode, context);
			}

			// Options modes use ctx.ui.custom(), which is TUI-only (RPC returns undefined).
			if (options.length > 0 && ctx.mode !== "tui") {
				return unavailableResult(params.question, mode, "ask-question with options requires interactive TUI mode", context);
			}

			if (!ctx.hasUI) {
				return unavailableResult(params.question, mode, "ask-question requires interactive mode UI", context);
			}

			return withUiLock(async () => {
				// The call may have been queued behind another popup.
				if (signal?.aborted) {
					return cancelledResult(params.question, mode, context);
				}
				...
				if (mode === "text") {
					...
					const answer = await ctx.ui.input(editorTitle, undefined, { signal });
					if (answer === undefined) {
						return cancelledResult(params.question, mode, context);
					}
		...
		renderResult(result, _options, theme) {
			const details = result.details as AskQuestionResultDetails | undefined;
			if (!details) {
			...
			if (details.status === "unavailable") {
				return new Text(theme.fg("warning", details.message || "ask-question unavailable"), 0, 0);
			}
